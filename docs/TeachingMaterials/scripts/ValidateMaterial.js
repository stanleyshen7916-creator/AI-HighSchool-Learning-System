/* docs/TeachingMaterials/scripts/ValidateMaterial.js — EO-S1.1-002 QA checklist's
   ...
   Validates materials/<materialId>/{metadata,manifest,summary,questionbank,related}.json,
   materials/<materialId>/material.md, and materials/<materialId>/source/ against
   schema/*.schema.json (Sprint AI-115 AI-115-02: questions.json renamed
   questionbank.json, material.md added — see that script's own header). */
"use strict";
const fs = require("fs");
...
/* Sprint AI-115 AI-115-02 Package Standard: material.md — the Claude-
   authored, human-readable rendering of this Package's content (real
   authored text, alongside metadata.json/summary.json/questionbank.json
   — not generated by any script, so its absence/emptiness is a real
   validation FAIL, unlike knowledge.json/report.md below). */
function validateMaterialMd() {
  console.log("\n[Package Standard — material.md]");
  const p = path.join(ROOT, "materials", materialId, "material.md");
  const exists = fs.existsSync(p);
  check("material.md exists", exists, p);
  if (exists) {
    const content = fs.readFileSync(p, "utf8").trim();
    check("material.md is non-empty", content.length > 0);
  }
}
...
const questionBank = validateFile("QuestionBank", "questionbank.json", "QuestionBank.schema.json");
...
/* Sprint AI-113 AI-807/AI-806: knowledge.json/report.md are OUTPUTS of
   RepositoryManager.js's prepare() step (derived from this same
   Package's own summary.json/questionbank.json — see that script's own
   header), not author-supplied inputs, so their absence here is never a
   validation FAIL — a freshly-authored Package that hasn't been prepared
   yet is still a valid Package. Reported informationally, plus the real
