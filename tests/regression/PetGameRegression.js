/* tests/regression/PetGameRegression.js — Sprint AI-134（真實 PO 需求）：
   ...
   - 成長機制：被動即時反映 — 寵物成長狀態直接對應
     ...
     操作「餵食」。
   - 寵物圖案：本專案沒有寵物美術素材，改用純 CSS/SVG 繪製
     （js/core/PetArt.js），提供 4 種固定圖案讓學生選擇。
   ...
   - AHS.StatisticsRuntime.overallMaterialCompletion()：Single Source —
     ...
     沒有教材時誠實回傳 0。
   - AHS.PetRuntime：4 種固定寵物圖案、選擇/持久化（跨頁面重新載入仍
   ...
   - AHS.PetWidget（真實 DOM 互動）：尚未選寵物時顯示 4 選項圖卡，點擊
...
console.log("\n[2] AHS.PetRuntime — 4 種固定寵物圖案，選擇後依 Workspace 持久化，無效 id 被拒絕");
{
  const { window } = loadPage("index.html");
  const AHS = window.AHS;

  const species = AHS.PetRuntime.listSpecies();
  check("真實提供 4 種寵物圖案", species.length === 4);
  const ids = species.map((s) => s.id).sort();
  check("圖案 id 真實為 cat/dog/rabbit/bird", JSON.stringify(ids) === JSON.stringify(["bird", "cat", "dog", "rabbit"]));

  check("尚未選擇前，getSelectedSpecies() 誠實為 null", AHS.PetRuntime.getSelectedSpecies() === null);

  check("selectSpecies('cat') 真實成功", AHS.PetRuntime.selectSpecies("cat") === true);
  check("選擇後 getSelectedSpecies() 真實變為 'cat'", AHS.PetRuntime.getSelectedSpecies() === "cat");

  check("selectSpecies('dragon')（不存在的圖案）真實被拒絕", AHS.PetRuntime.selectSpecies("dragon") === false);
  check("無效選擇不影響先前已選定的寵物", AHS.PetRuntime.getSelectedSpecies() === "cat");

  const carried = carriedSession(window);
  const { window: win2 } = loadPage("index.html", { seedSession: carried });
  check("重新載入頁面（真實跨 Session 持久化）後，選擇的寵物仍是 'cat'",
    win2.AHS.PetRuntime.getSelectedSpecies() === "cat");
}
...
console.log("\n[3] AHS.PetRuntime.growth() — percent 與既有 overallMaterialCompletion() 完全一致，stageIndex 分段正確");
{
  function stageCase(label, buildMaterials, expectedPercent, expectedStage) {
    const { window } = loadPage("index.html");
    const AHS = window.AHS;
    AHS.PetRuntime.selectSpecies("dog");
    buildMaterials(AHS);
...
console.log("\n[4] AHS.PetWidget（首頁）— 尚未選寵物顯示 4 選項，點擊即選定並切換成長畫面，可換一隻重選");
{
  const { window, consoleErrors } = loadPage("index.html");
  const doc = window.document;

  const options = doc.querySelectorAll(".pet-widget__option");
  check("尚未選寵物時，真實顯示 4 個可選圖案", options.length === 4);
  ...
  click(changeBtn);
  check("點擊「換一隻寵物」後，真實回到 4 選項的選擇畫面", doc.querySelectorAll(".pet-widget__option").length === 4);
...
console.log("\n[5] Hero Card 巧巧老師的小提醒 — 選寵物前維持原文案，選寵物後真實變為寵物飼養主題");
{
  ...
  winBefore.AHS.PetRuntime.selectSpecies("rabbit");
  ...
  check("提醒泡泡內文真實包含寵物名稱「兔子」", !!bubbleText && bubbleText.textContent.indexOf("兔子") !== -1);
