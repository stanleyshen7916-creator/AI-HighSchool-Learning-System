...
console.log("\n[2] Index 更新");
{
  ...
  /* Sprint AI-115 AI-115-01: an ARCHIVED Package (manifest.archived ===
     true, e.g. tm_6 after its 2026-09-07 chapter split into tm_14/tm_15)
     is a real tm_N directory (counted by listMaterialIds()) but is never
     included in generate()'s output — so the two counts diverging by
     exactly the archived count is the CORRECT behavior, not a bug. */
  const archivedCount = ids.filter(function (id) {
    var manifestPath = path.join(REPO, "docs/TeachingMaterials/materials", id, "manifest.json");
    try { return JSON.parse(fs.readFileSync(manifestPath, "utf8")).archived === true; } catch (e) { return false; }
  }).length;
  ...
  check("index.json 的教材數與實際掃描到的 Package 數一致（扣除已封存的 ARCHIVED Package）",
    indexJson.materials.length === count && count === ids.length - archivedCount);
